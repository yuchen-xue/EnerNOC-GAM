...
Name: EnerNOC GreenButton Data\
Source: [open enernoc data](https://open-enernoc-data.s3.amazonaws.com/anon/index.html)\
https://github.com/PetoLau/petolau.github.io/tree/master/_rmd\
Description: The raw data was provided by the EnerNOC electricity supplier. It contains anonymous 5-minute electricity consumption data for 100 commercial/industrial sites for the year 2012. The simplified version contains data at 30-minute intervals. The explanatory variables are:
...
ggplot(matrix_gam[1:row_Mon,], aes(1:row_Mon))+
           ...
           geom_line(aes(y = Predict, color = "Predict"), size = 0.8)
```
## This model can only predict the *trend* of the weekdays' electricity consumption, but fails in prediting the exact *amount* of electricity consumption. We do a detailed inspection on the electricity consumption on Monday.
...
## Next, we use another advanced method of interation and use a smooth function called "tensor product".
```{r}
...
```
These contour lines indicate each model's respons on `Weekly` and `Daily`. Althought they look similar, the contour of `gam_6` has more wave-like patterns. This is an indication of its higher sensitivity.
...
ggplot(data = datas, aes(date_time, value, group = type, colour = type)) +
  ...
  labs(x = "Time", y = "Load (kW)",
       title = "Fit from GAM n.6")
```
We can see that there is no weekly difference in the electricity consumption when the variable `Weekly` is droppped.
...
ggplot(data = datas, aes(date_time, value, group = type, colour = type)) +
  ...
  labs(x = "Time", y = "Load (kW)",
       title = "Fit from GAM n.6")
```
We can see that there is no difference in the electricity consumption over the 24 hours of a day when the variable `Daily` is droppped.
